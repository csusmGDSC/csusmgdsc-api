...
func InitRoutes(e *echo.Echo, h *handlers.Handler) {
	...
	// Comments
	e.POST("/comments", h.InsertCommentHandler)                // Create comment (Requires auth)
	...
	e.PUT("/comments/:id", h.UpdateCommentHandler)             // Update comment by id (Requires auth)
	e.DELETE("/comments/:id", h.DeleteCommentHandler)          // Delete comment by id (Requires auth)
}
